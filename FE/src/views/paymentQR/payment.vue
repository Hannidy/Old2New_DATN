<template>
  <div class="bg-light min-vh-100 d-flex flex-column">
    <main
      class="container flex-grow-1 d-flex align-items-center justify-content-center py-5"
    >
      <div
      ...
      >
        ...
        <div v-if="isExpired" class="card-body text-center px-4 py-5">
          <i
            class="bi bi-x-circle text-danger mb-3"
            style="font-size: 5rem"
          ></i>
          <h3 class="fw-bold text-dark">Mã QR đã hết hạn!</h3>
          <p class="text-muted">
            Bạn đã quá thời gian 5 phút để thanh toán.<br />Vui lòng thử lại
            sau.
          </p>
          <button
            @click="huyThanhToan"
            class="btn btn-primary px-5 py-2 mt-3 rounded-pill fw-bold"
          >
            Quay lại đơn hàng
          </button>
        </div>
        ...
        <div v-else-if="isSuccess" class="card-body text-center px-4 py-5">
          ...
          >
            Sàn O2N đã nhận được tiền thanh toán cho đơn hàng
          ...
          <div class="mt-5 mb-2">
            <button
              @click="huyThanhToan"
              class="btn btn-success py-3 px-5 fw-bold rounded-pill shadow-sm fs-6"
            >
        ...
        <div v-else>
          <div class="card-header bg-white text-center pt-5 pb-3 border-0">
            <h4 class="fw-bold text-dark mb-2">
              Thanh toán đơn hàng: #{{ donHangId }}
            </h4>
            <div
              class="badge bg-danger bg-gradient fs-5 px-4 py-2 mt-2 rounded-pill shadow-sm"
            >
              <i class="bi bi-stopwatch me-1"></i> Mã hết hạn trong:
              {{ formattedTime }}
            </div>
          </div>
            ...
            <div v-else>
              <div class="text-center mb-4">
                ...
                <img
                  :src="qrUrl"
                  alt="QR Thanh Toán"
                  class="img-fluid border rounded-4 p-2 shadow-sm"
                  style="max-width: 250px; background-color: #fff"
                />
              </div>

              <div class="bg-light p-3 rounded-4 mb-4 border shadow-sm">
                <div class="d-flex border-bottom pb-2 mb-2">
                  <div style="width: 130px" class="text-muted fw-bold">
                    Sản phẩm:
                  </div>
                  <div class="fw-semibold text-dark text-truncate">
                    {{ productNames || "Đang tải..." }}
                  </div>
                </div>
                ...
                <div class="d-flex align-items-center">
                  ...
                  <div
                    class="fw-bold text-primary fs-6 bg-white border px-2 py-1 rounded user-select-all"
                  >
                    {{ noiDungCK }}
                  </div>
                </div>
              ...
              <div
                class="d-flex justify-content-center align-items-center text-primary mb-4 fw-bold"
              >
                <span
...
const fetchPaymentInfo = async () => {
  try {
    ...
    const resDetails = await axios.get(
      `http://localhost:8080/api/don-hang/chi-tiet/${donHangId.value}`,
    );
    if (resDetails.data) {
      tongTien.value =
        resDetails.data.tongThanhTien || resDetails.data.tongTienHang || 0;
      if (resDetails.data.chiTietDonHangs) {
        productNames.value = resDetails.data.chiTietDonHangs
      ...
      const resQR = await axios.get(
        `http://localhost:8080/api/payment/qr-url/${donHangId.value}`,
      );
