  ...
  <div class="bg-light min-vh-100 pb-5">
    ...
    <main class="container main-content py-4">
                ...
                <table class="table table-hover align-middle mb-0">
                  <thead class="bg-white border-bottom">
                    <tr>
                      ...
                      <th
                        class="py-3 text-secondary small text-uppercase text-end pe-4"
                      >
                        Thành tiền
                      </th>
                    </tr>
            ...
            <div class="card-body p-4">
              ...
              >
                ...
                <span v-else class="fw-bold text-success"
                  >+ {{ formatCurrency(shippingFee) }}</span
                >
              </div>
      ...
      <div v-else class="text-center py-5 bg-white rounded-4 shadow-sm">
        ...
        <router-link to="/" class="btn btn-danger px-5 py-2 rounded-pill"
...
const selectAll = computed({
  ...
  set: (val) => {
    selectedItems.value = val ? cartItems.value.map((i) => i.id) : [];
  },
});
...
const removeItem = async (index) => {
  if (confirm("Xóa sản phẩm này?")) {
    ...
    const storedUser = JSON.parse(localStorage.getItem("user"));
    await axios.post(
      `http://localhost:8080/api/cart/${storedUser.id}`,
      cartItems.value,
      {
...
const fetchUserAddresses = async () => {
  ...
  try {
    const res = await axios.get(
      `http://localhost:8080/api/dia-chi/nguoi-dung/${storedUser.id}`,
      {
        headers: { Authorization: `Bearer ${storedUser.token}` },
      },
    );
    ...
    if (def) {
      selectedAddressId.value = def.diaChiId;
      calculateShippingFee();
    }
  } catch (error) {
    console.error(error);
...
const calculateShippingFee = async () => {
  if (selectedAddressId.value === "null" || selectedItems.value.length === 0) {
    shippingFee.value = 0;
    return;
  }
  isCalculatingFee.value = true;
  const addr = userAddresses.value.find(
  ...
  try {
    const res = await axios.post(
      "http://localhost:8080/api/ghn/fee",
      {
        fromDistrictId: 1454,
        fromWardCode: "21211",
        toDistrictId: addr.huyenCode,
        toWardCode: addr.phuongXaId,
...
const checkout = async () => {
  ...
  const storedUser = JSON.parse(localStorage.getItem("user"));
  const selectedOnes = cartItems.value.filter((i) =>
  ...
  const payload = {
    ...
    chiTietDonHangs: selectedOnes.map((i) => ({
      sanPhamId: i.id,
      soLuongMua: 1,
      giaLucMua: i.gia,
    })),
  ...
  try {
    ...
    const remain = cartItems.value.filter(
    ...
    if (remain.length > 0)
      ...
      await axios.delete(`http://localhost:8080/api/cart/${storedUser.id}`, {
        headers: { Authorization: `Bearer ${storedUser.token}` },
      });
