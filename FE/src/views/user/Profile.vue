<template>
  <div class="bg-light min-vh-100 pb-5">
    ...
    <!-- Thêm margin-top để nội dung không bị Header đè lên -->
            ...
            <div v-if="currentTab === 'profile'">
              ...
              >
                ...
                >
                  ...
                  >
                  <button
                    @click="$router.push('/rut-tien')"
                    class="btn btn-sm btn-light fw-bold rounded-pill px-4"
                  >
                    Rút tiền
                  </button>
                </div>
          ...
          <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <div class="modal-body p-4 text-center">
              <p class="small text-muted mb-4">
                Vui lòng nhập mã OTP để xác nhận (Mặc định: <b>123456</b>)
              </p>
              ...
              <div class="mb-3">
                <input
                  ...
                  class="form-control text-center fs-2 fw-bold border-danger py-2"
                  placeholder="------"
                  maxlength="6"
                  autofocus
                />
            ...
            <div class="modal-footer border-top-0 pt-0">
              <button
                class="btn btn-danger w-100 py-2 fw-bold shadow"
...
// 🔥 IMPORT HEADER VÀ SIDEBAR VÀO COMPONENT
...
const getAuthHeaders = () => {
  ...
  const token = userStr
    ? JSON.parse(userStr).token || JSON.parse(userStr).accessToken
...
const fetchProvinces = async () => {
  try {
    ...
    listProvinces.value = res.data.data.filter(
...
const setDefault = async (id) => {
  try {
    await axios.put(
      `http://localhost:8080/api/dia-chi/${currentUserId}/default/${id}`,
      {},
      getAuthHeaders(),
    );
